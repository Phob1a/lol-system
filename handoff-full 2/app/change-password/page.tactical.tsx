...
import { ChangePasswordForm } from '@/components/auth/ChangePasswordForm.tactical';
      ...
      <div className="tc-card" style={{ width: 460, padding: 32, position: 'relative' }}>
        ...
        <header style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
          <div style={{ width: 4, height: 28,
            background: session.user.mustChangePwd ? 'var(--tc-red)' : 'var(--tc-cyan)',
            boxShadow: `0 0 12px ${session.user.mustChangePwd ? 'var(--tc-red)' : 'var(--tc-cyan)'}` }} />
          <div>
            <div className="tc-h1" style={{ fontSize: 20 }}>
              CHANGE<span style={{ color: 'var(--tc-cyan)' }}>//</span>PASSWORD
            </div>
            <div className="tc-label">
              {session.user.mustChangePwd ? '⚠ FIRST LOGIN · ROTATION REQUIRED' : 'UPDATE TERMINAL CREDENTIAL'}
            </div>
          </div>
        </header>

        {session.user.mustChangePwd && (
          <div style={{ margin: '14px 0 18px', padding: '8px 12px',
