...
/** Tactical · 选手新建/编辑弹窗
 *  对外 props 与原版一致：{ open, onOpenChange, onSaved, mode, player? }
 *  保留所有 fetch（POST /api/players, PATCH /api/players/[id]）与字段名。
 */
...
export function PlayerFormDialog({ open, onOpenChange, onSaved, mode, player }: Props) {
  ...
  const defaultValues: PlayerInputType = mode === 'edit'
    ? {
    ...
    : {
        ...
        cost: 0, isCaptain: false, isRetired: false,
      };

  const form = useForm<PlayerInputType>({
    resolver: zodResolver(PlayerInput), defaultValues,
  });

  useEffect(() => { form.reset(defaultValues); /* eslint-disable-next-line */ }, [open, mode, player?.id]);
  ...
  async function onSubmit(values: PlayerInputType) {
    ...
    if (!res.ok) { toast.error(body.error ?? '保存失败'); return; }
  ...
  function togglePos(field: 'primaryPositions' | 'secondaryPositions',
                     pos: PlayerInputType['primaryPositions'][number]) {
    ...
    <div onClick={() => onOpenChange(false)}
      ...
      <div onClick={e => e.stopPropagation()} className="tc-card"
        ...
        <header style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
          <div style={{ width: 4, height: 24, background: 'var(--tc-cyan)',
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <Field label="GAME ID *" disabled={mode === 'edit'}
          ...
          <PosGroup label="PRIMARY POSITIONS *" selected={primary ?? []}
            onToggle={p => togglePos('primaryPositions', p)}
            error={form.formState.errors.primaryPositions?.message as string | undefined} />

          <PosGroup label="SECONDARY POSITIONS" selected={secondary ?? []}
            onToggle={p => togglePos('secondaryPositions', p)} muted />

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            <Field label="COST *" type="number"
              {...form.register('cost', { valueAsNumber: true })}
              error={form.formState.errors.cost?.message} />
            <Toggle label="CAPTAIN" checked={form.watch('isCaptain')}
              onChange={v => form.setValue('isCaptain', v)} accent="var(--tc-cyan)" />
            <Toggle label="RETIRED" checked={form.watch('isRetired')}
              onChange={v => form.setValue('isRetired', v)} accent="var(--tc-red)" />
          </div>
...
const Field = ({ label, error, ...rest }:
  ...
  <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
    ...
    <input {...rest} className="tc-mono"
      style={{ background: 'var(--tc-bg-0)', color: 'var(--tc-text)',
        border: '1px solid var(--tc-line2)', padding: '7px 10px',
...
const Toggle = ({ label, checked, onChange, accent }: {
  ...
  <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
    ...
    <button type="button" onClick={() => onChange(!checked)}
      style={{
        padding: '7px 10px', fontFamily: 'var(--tc-font-mono)', fontSize: 11,
...
function PosGroup({ label, selected, onToggle, error, muted }: {
  label: string; selected: string[]; onToggle: (p: any) => void;
  error?: string; muted?: boolean;
}) {
  return (
    <div>
      <span className="tc-label">{label}</span>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 6 }}>
        {POSITION_OPTIONS.map(p => {
          ...
          return (
            <button key={p.value} type="button" onClick={() => onToggle(p.value)}
              ...
              {on ? '◆ ' : '○ '}{p.value}<span style={{ marginLeft: 4, opacity: 0.55 }}>{p.label}</span>
            </button>
          );
        })}
      </div>
