...
/** Tactical · CSV / XLSX 批量导入
 *  保留 POST /api/players/import 与返回 schema {totalRows,inserted,updated,skipped[],duplicates[],truncated}
 */
...
export function ImportUpload() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!(file instanceof File) || file.size === 0) { toast.error('请选择文件'); return; }
    setSubmitting(true); setResult(null);
    const res = await fetch('/api/players/import', { method: 'POST', body: data });
    const body = await res.json().catch(() => ({}));
    setSubmitting(false);
    if (!res.ok) { toast.error(body.error ?? '导入失败'); return; }
      ...
      <div className="tc-card" style={{ padding: 18 }}>
        ...
        <form onSubmit={onSubmit} style={{ display: 'flex', gap: 8, alignItems: 'stretch' }}>
          <input type="file" name="file" accept=".csv,.xlsx,.xls" required
            style={{
              flex: 1, background: 'var(--tc-bg-0)', color: 'var(--tc-text)',
              border: '1px dashed var(--tc-line2)', padding: '9px 12px',
              fontFamily: 'var(--tc-font-mono)', fontSize: 11,
            }} />
        ...
        <details style={{ marginTop: 14 }}>
          <summary className="tc-label" style={{ cursor: 'pointer' }}>
            ▸ COLUMN SCHEMA
          </summary>
          <ul className="tc-mono" style={{ fontSize: 11, color: 'var(--tc-text-dim)',
            marginTop: 8, paddingLeft: 18, lineHeight: 1.7 }}>
      ...
      {result && (
        <div className="tc-card" style={{ padding: 18 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            ...
            {result.truncated && <span className="tc-chip" style={{
              background: 'rgba(255,178,61,0.14)', color: 'var(--tc-amber)',
              borderColor: 'var(--tc-amber)' }}>⚠ TRUNCATED @ 5000 ROWS</span>}
          </div>
          ...
          {result.duplicates.length > 0 && (
            <div style={{ marginTop: 14, padding: 10,
              ...
              <div className="tc-label" style={{ color: 'var(--tc-amber)' }}>
                IN-FILE DUPLICATES (LATER WINS)
              </div>
              <ul className="tc-mono" style={{ fontSize: 11, color: 'var(--tc-text)',
                marginTop: 6, paddingLeft: 18 }}>
                {result.duplicates.map(d => (
                  <li key={d.gameId}><code style={{ color: 'var(--tc-amber)' }}>{d.gameId}</code>
                    {' · rows '}{d.rows.join(', ')}</li>
                ))}
              </ul>
            </div>
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <div style={{ marginTop: 6, border: '1px solid var(--tc-line2)' }}>
                <div style={{
                  ...
                  background: 'rgba(255,61,92,0.08)', padding: '5px 10px',
                ...
                {result.skipped.map((s, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                      {s.errors.map((err, j) => (
                        <span key={j} className="tc-chip" style={{
                          background: 'rgba(255,61,92,0.14)',
                          color: 'var(--tc-red)', borderColor: 'var(--tc-red)' }}>{err}</span>
                      ))}
                    </span>
                  </div>
                ))}
...
function Stat({ label, value, accent }: { label: string; value: number; accent: string }) {
  return (
    <div style={{ padding: '10px 12px', border: '1px solid var(--tc-line)',
