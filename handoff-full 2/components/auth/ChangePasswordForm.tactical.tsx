...
/** Tactical · 修改密码（保留 POST /api/auth/change-password 与字段名） */
...
export function ChangePasswordForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(false);
    if (!res.ok) { toast.error(body.error ?? '修改失败'); return; }
    toast.success('密码已更新');
  ...
  const strengthColor = ['var(--tc-line2)','var(--tc-red)','var(--tc-amber)','var(--tc-cyan)','var(--tc-green)'][strength];
  ...
  return (
    <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div>
        <PwField name="newPassword" label="NEW PASSWORD (≥6)" autoComplete="new-password" minLength={6} required
      ...
      <button type="submit" className="tc-btn tc-btn-primary"
        style={{ justifyContent: 'center', marginTop: 6 }} disabled={submitting}>
