...
- 内战样本小，若 `sourceGames < 3` 或同口径样本过少，UI 必须提示“小样本，仅供参考”，不能把分位分当成稳定实力分。

### 4.12 第一版图表边界

第一版只展示现有 `games/{id}` 详情能稳定支撑的图表。

保留：

- 伤害构成堆叠条：`physicalDamageDealtToChampions` / `magicDamageDealtToChampions` / `trueDamageDealtToChampions`。
- 六边形能力图：按 4.11 的赛事内分位口径。
- 输出/视野趋势：每场 `totalDamageDealtToChampions` 和 `visionScore` 可以画趋势，但必须各自归一化到 0-100 或拆成双轴；不能把“2 万伤害”和“30 视野分”直接放进同一原始 Y 轴。少于 3 场时降级成逐场数字。

删除或推迟：

- 高光时间线：当前 LCU match-history stats 没有事件时间戳，`firstBloodKill` / `firstTowerKill` 只是布尔值，不能显示“06:12”这类时间。
- 大龙/小龙伤害峰值：当前 stats 只有 `damageDealtToObjectives` 总量，没有大龙/小龙专属伤害字段，也没有峰值时间。
- 对局热力图：第一版推迟。若后续要做，必须固定单一指标，例如“对英雄伤害赛事内分位”，不能混用胜负、MVP、伤害作为同一种颜色强度。
...
- 趋势图必须明确轴和归一化口径；没有 3 场数据时不画连线。
- 不在第一版 UI 中渲染高光时间线或“大龙伤害峰值”。
